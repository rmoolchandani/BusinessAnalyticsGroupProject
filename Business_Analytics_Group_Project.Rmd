...
library(rpart.plot) #fro decision tree
...
Churn_Train <- read.csv("~/Desktop/Churn_Train.csv")
summary(Churn_Train)
...
>>>>>>> 1b62a7b33c87206966f144e4a292185b6e1da422
```{r}
set.seed(2020)
...
#### Lets make a confusion matrix for the logistic regression performed above (with states column /without states column)
...
company then targets those customers with special programs or incentives. Therefore Accuracy is the top criteria for the model selection.
...
#rpart () used on the decision tree
dcplot<-rpart(Churn_Train$churn ~.,data=Churn_Train[,-1],method='class')
...
# Predicting the Churn results based on the model
Churn_Prob <- predict(Model_ABC_Wireless, Customers_To_Predict)

#Results of pred_tree
table(Churn_Prob) # 0=900, customer not likely to churn ,1=100 ,customer likely to churn 

#plotting the prediction results :
plot(Churn_Prob,xlab="1= customer likely to churn ,0= customer will not churn" , ylab="Customers",main="Prediction of customers who are likely to churn",col="orange")
